...
function Feed() {
	...
	return rooms?.map((room) => (
		<section key={room.slug} className="roomListRoom">
			<header className="roomListRoom__header">
				<Link href={`user/profile/${room.host?.id}`} className="roomListRoom__author">
					<div className="avatar avatar--small">
						{/* <img src="{{ room.host.avatar.url }}" /> TODO*/}
			...
			<article className="roomListRoom__content">
				<Link href={`rooms/${room.slug}/`}>{room.name}</Link>
			</article>
			<section className="roomListRoom__meta">
				<Link
